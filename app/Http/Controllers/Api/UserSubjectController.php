...
{
    /**
     * ✅ [الخطوة 0: بعد التسجيل مباشرة]
     * يجلب قائمة بكل المواد المتاحة في الجامعة لعرضها للطالب.
     */
    public function getAllSubjectsForInitialSetup()
    {
        // جلب جميع المواد مع الحقول الأساسية فقط
        $subjects = Subject::all(['id', 'name', 'hour_count']);
        return response()->json(['subjects' => $subjects]);
    }

    /**
     * [الخطوة 1: بعد اختيار الطالب للمواد المكتملة]
     * يستقبل قائمة المواد التي أكملها الطالب سابقًا لحساب الساعات الأولية.
     */
    public function submitInitialCompletedSubjects(Request $request)
    {
        ...
        // هذه الدالة تفترض أن الطالب ليس لديه فصل دراسي بعد
